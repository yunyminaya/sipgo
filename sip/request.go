...
// inviteRequest is the original dialog-establishing INVITE request,
// inviteResponse is the 2xx INVITE response, which should contain a permanent routing set, if any.
// isOriginalUAC should be true if newReq is being generated by the UAC (og sender) to be sent to UAS (og receiver)
func CopyRoutingHeaders(newReq *Request, inviteRequest *Request, inviteResponse *Response, isOriginalUAC bool) {
	if newReq == nil {
		return
	}
	var headers []Header
	reverse := isOriginalUAC
	...
	for _, header := range headers {
		newReq.AppendHeader(sipgo.NewHeader("Route", header.Value()))
	}
}
